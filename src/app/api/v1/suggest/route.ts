...
export async function GET(request: NextRequest) {
  ...
  const cachedData = cache.get<SuggestResponse['suggestions']>(cacheKey);
  if (cachedData) {
    return NextResponse.json({ suggestions: cachedData.value });
  }
  ...
  cache.set(cacheKey, finalSuggestions, 3600 * 1000); // Cache for 1 hour
