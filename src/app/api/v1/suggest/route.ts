...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = await hashCacheKey(`suggest:${trimmed}:${limit}`);
  const cached = cache.get<{ suggestions: SuggestResponse['suggestions'] }>(cacheKey);
  if (cached) {
  ...
  cache.set(cacheKey, { suggestions: finalSuggestions }, 5 * 60 * 1000); // 5 minutes TTL
