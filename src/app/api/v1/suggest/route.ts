...
import { cache, hashCacheKey } from '@/lib/cache';
...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = await hashCacheKey(`suggest_${trimmed}_${limit}`);
  const cachedResult = cache.get<SuggestResponse['suggestions']>(cacheKey);
  if (cachedResult) {
    return NextResponse.json({ suggestions: cachedResult.value });
  }
  ...
  // Cache the final suggestions for 1 hour
  cache.set(cacheKey, finalSuggestions, 60 * 60 * 1000);
