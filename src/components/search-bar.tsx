    ...
    <div ref={containerRef} className={`relative w-full ${compact ? 'max-w-2xl' : 'max-w-3xl'}`}>
      <div className="relative">
        ...
        {isLoading ? (
          ...
          <button
            type="button"
            className="absolute right-4 top-1/2 -translate-y-1/2 p-0.5 text-muted-foreground hover:text-[#1C1C1E] transition-colors rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#007AFF]"
            onClick={() => {
              setQuery('');
              setSuggestions([]);
              setShowSuggestions(false);
