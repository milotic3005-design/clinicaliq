...
class MemoryCache {
  ...
  set<T>(key: string, value: T, ttlMs: number = DEFAULT_TTL_MS): void {
    // If the cache is full, delete the oldest entry (the first item in the Map)
    ...
    // Setting or re-setting updates the value and puts it at the end of the Map
    // Map.set does not update insertion order for existing keys, so we must delete first
  ...
  get<T>(key: string): { value: T; age_ms: number } | null {
    ...
    // Re-insert to refresh recency in insertion order for LRU
    this.store.delete(key);
